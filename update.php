...
?>
<form action='update.php?id=<?php echo $manv;?>' method="post" enctype="multipart/form-data">
    ...
    ẢNH MỚI (để cập nhật): <input type="file" name="anh_moi" value='<?php echo $rows[8]; ?>'><br>
...
<?php
	...
	if (isset($_POST['btnSua']))
	{
        ...
        if (!empty($anh_moi)) {
            ...
            $target_moi = "C:\\xampp\\htdocs\\backend\\img\\" . basename($anh_moi);

            $allowed_extensions = array('jpg', 'jpeg', 'png', 'gif');
            $file_extension_moi = pathinfo($_FILES['anh_moi']['name'], PATHINFO_EXTENSION);

            if (!in_array(strtolower($file_extension_moi), $allowed_extensions)) {
                echo "Chỉ chấp nhận các file có định dạng JPG, JPEG, PNG hoặc GIF.";
                exit; // Dừng xử lý nếu định dạng không hợp lệ
            }

            if (move_uploaded_file($_FILES['anh_moi']['tmp_name'], $target_moi)) {
                // Nếu upload file mới thành công, cập nhật tên ảnh
                $anh = $anh_moi;
            } else {
                echo "Lỗi khi di chuyển tệp mới.";
            }
        }
